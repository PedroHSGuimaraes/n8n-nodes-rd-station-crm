...
/**
 * Classe para o nó de Equipes do RD Station CRM
 * Este nó permite consultar as equipes cadastradas no RD Station CRM.
 * 
 * As equipes são grupos de usuários dentro do CRM (por exemplo, equipe de vendas regionais,
 * time de produtos específicos, etc.) e servem principalmente para filtrar visibilidade
 * de negócios e contatos por equipe.
 * 
 * Normalmente, a API só permite operações de leitura para equipes, já que a criação e
 * gerenciamento são feitas na interface administrativa do CRM.
 */
export class RdStationCrmTeams implements INodeType {
	description: INodeTypeDescription = {
		// Informações básicas do nó que serão exibidas na interface do n8n
		displayName: 'RD Station CRM Equipes',
		name: 'rdStationCrmTeams',
		...
		description: 'Consultar equipes no RD Station CRM', // Descrição do nó
		defaults: {
			name: 'RD Station CRM Equipes', // Nome padrão quando o nó é adicionado
		},
		...
		properties: [
			// Seleção do recurso (neste caso, só temos equipes)
			{
				...
				options: [
					{
						name: 'Equipe', // Nome amigável da opção
						value: 'team', // Valor interno da opção
					},
				],
				default: 'team', // Valor padrão selecionado
				required: true, // Campo obrigatório
			},
			// Seleção da operação a ser realizada sobre as equipes
			// Note que para equipes, geralmente só há operações de consulta
			{
				...
				displayOptions: {
					show: {
						// Mostra este campo apenas quando o recurso for 'team'
						resource: [
							'team',
						],
					},
			...
			// Campos para a operação OBTER EQUIPE
			{
				displayName: 'ID da Equipe', // Campo para informar ID da equipe
				name: 'teamId',
			...
			// Campos para a operação LISTAR EQUIPES
			{
				...
				displayOptions: {
					show: {
						// Mostra apenas quando recurso é 'team' e operação é 'getAll'
						resource: [
							'team',
						],
						operation: [
				...
				options: [
					// Opções de filtragem e paginação para listagem
					{
						...
						type: 'number',
						default: 50,
						description: 'Número máximo de equipes a retornar',
					},
					{
					...
					{
						displayName: 'Status', // Filtrar por status da equipe
						name: 'status',
						type: 'options',
						options: [
							{
								name: 'Ativa', // Equipes ativas
								value: 'active',
							},
							{
								name: 'Inativa', // Equipes desativadas
								value: 'inactive',
							},
							{
						...
						default: 'active',
						description: 'Filtrar equipes por status',
					},
				],
			},
		],
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				if (resource === 'team') {
					if (operation === 'getAll') {
						// Operação: LISTAR EQUIPES
						
						// Preparando a requisição para listar equipes
						options.method = 'GET'; // Método HTTP GET para listar
						options.uri = `${baseUrl}/teams`; // Endpoint de equipes
						...
						// Operação: OBTER EQUIPE ESPECÍFICA
						
						// Obtém o ID da equipe a ser consultada
						const teamId = this.getNodeParameter('teamId', i) as string;
						
						// Verifica se o ID da equipe foi fornecido
						if (!teamId) {
							throw new NodeOperationError(this.getNode(), 'É necessário fornecer um ID de equipe válido!', { itemIndex: i });
						}
						
						// Configurando a requisição para buscar uma equipe específica
						options.method = 'GET';
						options.uri = `${baseUrl}/teams/${teamId}`; // Endpoint com ID da equipe
					}
				}
